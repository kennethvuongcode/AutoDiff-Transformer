...
def transformer(X: ad.Node, nodes: List[ad.Node], 
    ...
    print("got params")
    # Defining single_head_attention
    Q = ad.matmul(X,W_q) # Shape (50,28,128)
    K = ad.matmul(X,W_k) # (50,28,128)
    V = ad.matmul(X,W_v) # (50,28,128)

    attention_scores = ad.matmul(Q,ad.transpose(K,-1,-2)) 
    attention_norm = ad.div_by_const(attention_scores,model_dim**0.5)
    attention = ad.softmax(attention_norm) # Shape (50,28,28)

    print("got attention")
    output = ad.matmul(attention,V) # (50,28,28) @ (50,28,128) = (50,28,128)
    sh_att = ad.matmul(output,W_o) # (50,28,128) @ (50,128,128) = (50,28,128)
    sh_att = ad.div_by_const(ad.sum_op(sh_att,dim=[1]),seq_length) # reducing dimensionality to (50,128)
    
    print("got shatt")
    # Computing Layernorm, no residual add
    norm_shatt = ad.layernorm(sh_att,[model_dim],eps) # Layernorm keeps dimension (50,128)

    print("got layernorm1")
    # Computing feed forward  
    ff_h = ad.matmul(norm_shatt,W_1) + ad.broadcast(b_1,(model_dim,),(batch_size,model_dim)) #(50,128)
    ff_o = ad.matmul(ad.relu(ff_h),W_2) + ad.broadcast(b_2,(num_classes,),(batch_size,num_classes)) #(50,10)

    print("got layernorm")
    # Computing Layernorm, no residual add
    norm_ff = ad.layernorm(ff_o,[num_classes],eps) #(50,10)

    print("got transformer")
    return norm_ff #(50,10)
...
def softmax_loss(Z: ad.Node, y_one_hot: ad.Node, batch_size: int) -> ad.Node:
    ...
    """
    """TODO: Your code here"""
    sm_prob = ad.softmax(Z,dim=-1) #(50,10)

    log_sm_prob = ad.log(sm_prob)
    
    loss_vec = ad.mul_by_const(ad.sum_op(ad.mul(y_one_hot,log_sm_prob),dim=[1]),-1)

    return ad.div_by_const(ad.sum_op(loss_vec, dim=[-1]), batch_size) #returns a scalar
...
def sgd_epoch(
    ...
    for i in range(num_batches):
        ...
        for i in range(len(model_weights)):
            ...
            model_weights[i] -= lr * gradients[i].sum(dim = 0)
...
def train_model():
    ...
    grads: List[ad.Node] = ad.gradients(loss,[W_Q, W_K, W_V, W_O, W_1, W_2, b_1, b_2]) # TODO: Define the gradient nodes here
    ...
    def f_run_model(model_weights,X_train,y_train):
    ...
    model_weights: List[torch.Tensor] = [torch.tensor(W_Q_val),
